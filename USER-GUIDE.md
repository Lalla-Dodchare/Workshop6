...
- ใช้แผนภาพง่ายๆ เช่น:
  ...
  file3.png  ─┘
  ```
- เปรียบเทียบกับโค้ดที่เขียนมาแล้ว เช่น "เหมือน route /files บรรทัด xxx เลย แค่เปลี่ยนจาก อ่านรายการ เป็น บีบอัด"
...
- [x] แก้ upload แยกโฟลเดอร์ตาม user id (server.js)
- [x] DELETE /files/:filename — user + admin
- [x] GET /files แบบ admin (เห็นไฟล์ทุก user + owner)
- [x] Download แบบ admin (/download/:owner/:filename)
- [x] DELETE แบบ admin (/files/:owner/:filename)
- [x] หน้า User — layout HTML + Tailwind (page/user/home.html)
- [x] user.js — upload/preview/list/download/delete/logout (inline ใน home.html)
- [x] Preview รูปก่อน upload (FileReader)
- [x] ยืนยันก่อนลบ (confirm)
- [x] POST /register + สมัครสมาชิก + เช็ค username ซ้ำ
...
### 1. ระบบสมัครสมาชิก (Register) — ✅ เสร็จแล้ว
- ~~เพิ่ม form ใน index.html~~
- ~~เพิ่ม `POST /register` ใน server.js~~
- ~~เช็ค username ซ้ำก่อน push~~

### 2. Preview ไฟล์แบบละเอียด — 🔨 กำลังทำ

**สิ่งที่อยากได้:**
- ตารางไฟล์แสดง thumbnail เล็กๆ + ข้อมูลไฟล์ (ชื่อ, ขนาด, ประเภท, วันที่อัปโหลด)
- ดับเบิ้ลคลิก → เปิด modal ดูแบบเต็มจอ พร้อมข้อมูลละเอียด (คล้าย macOS file info)
- Preview ตามประเภทไฟล์: รูป → แสดงรูป, PDF → iframe, อื่นๆ → ไอคอน
- รูปภาพแสดง dimensions (กว้าง x สูง px) ด้วย

**ไอเดียเพิ่มเติม:**
- Zoom เข้า/ออก ในรูปใน modal
- Slideshow — กดลูกศรซ้าย/ขวาดูไฟล์ถัดไปใน modal
- Drag & Drop upload — ลากไฟล์มาวางแทนการกด choose file

**ติดปัญหา / ต้องทำก่อน:**
- `GET /files` ฝั่ง user ส่งแค่ชื่อไฟล์ (string) → ต้องแก้ให้ส่ง object ที่มี size, type, birthtime, dimensions มาด้วย
- ต้องใช้ `fs.statSync()` ดึง size + birthtime
- ต้องใช้ library `image-size` (npm) ดึง dimensions เฉพาะรูป
- home.html ต้องแก้ตาราง + เพิ่ม modal HTML + เพิ่ม JS จัดการ dblclick/modal

**ลำดับที่ควรทำ:**
1. `npm install image-size`
2. แก้ `GET /files` ใน server.js ให้ส่ง { filename, size, type, birthtime, dimensions }
3. แก้ home.html — ตารางแสดง thumbnail + ข้อมูลย่อ
4. เพิ่ม modal HTML + CSS (Tailwind)
5. เพิ่ม JS — dblclick เปิด modal, แสดงข้อมูลละเอียด, preview ตามประเภทไฟล์
6. เพิ่ม Zoom / Slideshow / Drag & Drop (ถ้ามีเวลา)
...
### 4. Filter ชื่อไฟล์ — ✅ เสร็จแล้ว (ฝั่ง admin)

### 5. UI สวย
- เพิ่ม hover effect บนแถวตาราง
- ปุ่ม Download/ลบ ใส่ Tailwind class สีสัน
- แสดงขนาดไฟล์ในตาราง (จะได้จากการแก้ GET /files)
